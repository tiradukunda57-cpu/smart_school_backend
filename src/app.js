    ...
    for (const table of requiredTables) {
      if (existingTables.includes(table)) {
        try {
          ...
          console.log(
            `   │ ${table.padEnd(12)} │ ${String(count).padStart(8)} │`,
          );
        } catch {
...
app.get("/api/docs", (req, res) => {
  const docs = {
    ...
    version: "1.0.0",
    description:
      "Complete REST API for managing students, teachers, attendance, assignments, notes, and messaging.",
    ...
    roles: {
      teacher: {
        description:
          "Full CRUD access to students, attendance, assignments, notes. Can view/send messages.",
        permissions: ["create", "read", "update", "delete"],
      },
      student: {
        description:
          "Read-only access to attendance, assignments, notes. Can view teachers and send messages.",
        permissions: ["read"],
      },
    },
        ...
        routes: [
          {
            ...
            access: "Teacher only",
            query: { optional: ["search", "level", "page", "limit"] },
        ...
        routes: [
          {
            ...
            access: "Teacher only",
            query: {
              optional: ["search", "status", "date", "course", "page", "limit"],
            },
          ...
          },
          {
            ...
            access: "Teacher only",
            body: {
              required: ["student_id", "status", "date"],
              optional: ["course", "note"],
            },
          ...
          },
          {
            ...
            access: "Authenticated",
            response:
              "200 — { summary: { present, absent, late, excused, total } }",
          },
        ],
      },
        ...
        routes: [
          ...
          },
          {
            ...
            body: {
              ...
              optional: ["course", "due_date", "priority"],
            },
      ...
      notes: {
        ...
        routes: [
          ...
          },
          {
            ...
            access: "Teacher only",
            body: { optional: ["title", "content", "course", "category"] },
      ...
      messages: {
        ...
        routes: [
          ...
          },
          {
            ...
            description:
              "Get chat history with a specific user (auto marks as read)",
            access: "Authenticated",
...
const routeMounts = [
  {
    path: "/api/auth",
    router: authRoutes,
    label: "Auth (login/register/profile)",
  },
  {
    path: "/api/admin",
    router: adminRoutes,
    label: "Admin (teacher approval, users)",
  },
  {
    path: "/api/quizzes",
    router: quizRoutes,
    label: "Quizzes (create/take/track)",
  },
  { path: "/api/groups", router: groupRoutes, label: "Groups (chat/manage)" },
  {
    path: "/api/students",
    router: studentRoutes,
    label: "Students (CRUD — teacher only)",
  },
  {
    path: "/api/recovery",
    router: recoveryRoutes,
    label: "Recovery (password requests)",
  },
  {
    path: "/api/teachers",
    router: teacherRoutes,
    label: "Teachers (list/profile)",
  },
  {
    path: "/api/attendance",
    router: attendanceRoutes,
    label: "Attendance (bulk/single/my)",
  },
  {
    path: "/api/assignments",
    router: assignmentRoutes,
    label: "Assignments (broadcast)",
  },
  { path: "/api/notes", router: noteRoutes, label: "Notes (publish/download)" },
  {
    path: "/api/messages",
    router: messageRoutes,
    label: "Messages (conversations/chat)",
  },
];
...
async function startServer() {
  ...
  Object.values(endpointCounts).forEach((c) => {
    totalEndpoints += c;
  });
...
// ── Auto-cleanup rejected teachers every hour ────────────────
setInterval(async () => {
  try {
    ...
    for (const row of result.rows) {
      await dbQuery('DELETE FROM users WHERE id = $1', [row.user_id])
    }

    if (result.rows.length > 0) {
      console.log(`🗑️  Auto-deleted ${result.rows.length} rejected teacher(s)`)
    }
  } catch (err) {
...
// ── Auto-archive daily report at midnight ────────────────────
const scheduleDailyReport = () => {
  ...
  setTimeout(async () => {
    try {
      ...
      // Generate report data inline
      const data = {}
      const counts = ['users', 'students', 'teachers', 'assignments', 'quizzes', 'attendance', 'messages']
      for (const table of counts) {
        const r = await dbQuery(`SELECT COUNT(*) FROM ${table}`)
        data[table] = parseInt(r.rows[0].count)
      }
      ...
      await dbQuery(
        ...
        [reportDate, JSON.stringify(data)]
      )

      console.log(`📊 Daily report archived for ${reportDate}`)
    } catch (err) {
